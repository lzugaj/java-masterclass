...
public class LargestPrime {
    ...
    private static int getLargestPrime(int number) {
        int resultOfMultiplyingPrimeFactors = 1;
        int largestPrimeDivider = 0;
        int howManyDividers;
        int reminderNumber;
        if (number > 1) {
            for (int numberDivider = 2; numberDivider <= number; numberDivider++) {
                if ((number % numberDivider) == 0) {
                    howManyDividers = 0;
                    for (int i = 1; i <= numberDivider; i++) {
                        reminderNumber = numberDivider % i;
                        if (reminderNumber == 0) {
                            howManyDividers++;
                        }
                    }
                    ...
                    if (howManyDividers == 2) {
                        resultOfMultiplyingPrimeFactors = resultOfMultiplyingPrimeFactors * numberDivider;
                        largestPrimeDivider = numberDivider;
                    }
                }
            }

            return largestPrimeDivider;
        }

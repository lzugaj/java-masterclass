...
public class NumberPalindrome {
    ...
    public static void main(String[] args) {
         ...
         * TIP: A palindrome number is a number which when reversed is equal to the original number.
         * TIP: For example: 121, 12321, 1001 etc.
    ...
    private static boolean isPalindrome(int number) {
        int absoluteNumber = calculateAbsoluteValue(number);
        int copyOfOriginalNumber = absoluteNumber;
        int reverseNumber = 0;
        while (copyOfOriginalNumber > 0) {
            int lastDigit = copyOfOriginalNumber % 10;
            copyOfOriginalNumber /= 10;
            reverseNumber = (reverseNumber * 10) + lastDigit;
        }

        return absoluteNumber == reverseNumber;
    }

    private static int calculateAbsoluteValue(int number) {
        if (number < 0) {
